...
- **`log_experiment`** — records result. `keep` auto-commits. `discard`/`crash`/`checks_failed` auto-reverts code changes (autoresearch files preserved). Always include secondary `metrics` dict. Dashboard: ctrl+shift+e.
...
5. Add `autoresearch.progress.md` to `.gitignore` — it is a transient live-progress file written while an experiment runs and removed when it finishes; it should never be committed.
